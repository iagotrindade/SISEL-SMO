...
<?php if (isset($_SESSION['mensagem']) && $_SESSION['mensagem']): ?>
    <div class="container mt-3">
        <div class="alert alert-success alert-dismissible fade show" role="alert">
            <i class="fas fa-check-circle me-2"></i><?php echo $_SESSION['mensagem']; $_SESSION['mensagem'] = null; ?>
...
<section id="contact" class="contact">
    ...
    <div class="card shadow-sm mb-4" id="filtros-container">
        ...
        <div class="card-body">
            ...
            <div class="col-lg-6">
                <label class="form-label fw-semibold">Local Designação Compareceu</label>
                <select id="local_compareceu_designacao_filtro" name="local_compareceu_designacao_filtro" style="width: 100%" class="chosen-select">
                    <option value="">Selecione a Opção</option>
                    <?php
                    foreach ($todas_cid as $value) {
                        echo "<option value='" . htmlspecialchars($value['nome']) . "'>" . htmlspecialchars($value['nome']) . '/' . htmlspecialchars($value['uf']) . "</option>";
                    }
                    ?>
                </select>
            </div>
            ...
            <div class="col-lg-6">
                <label class="form-label fw-semibold">Incorporação - Compareceu?</label>
                <select id="incorporacao_filtro" name="incorporacao_filtro[]" style="width: 100%" class="chosen-select" multiple>
                    ...
                    foreach ($todas_incorporacao as $value) {
                        echo "<option value='" . htmlspecialchars($value['incorporacao']) . "'>" . trata_data($value['incorporacao']) . "</option>";
                    }
                    ?>
                </select>
            </div>
            ...
            <div class="col-lg-2">
                ...
                <select id="voluntario_filtro" name="voluntario_filtro" style="width: 100%" class="chosen-select">
    ...
    <form method="post" action="controller/pre_disposicao_atualiza.php">
        <table id="tabela_dinamica" class="table table-hover" style="width:100%; font-size: 10px;">
            <thead>
                <tr>
                    <th>#</th>
                    <th>Nome</th>
        ...
        <div class="row">
            <div class="col-lg-4">
                <select name="id_om_1_fase" style="width: 100%" class="chosen-select">
                    ...
                    <?php
                    foreach ($todas_oms_1_fase as $value) {
                        echo "<option value='" . htmlspecialchars($value['id']) . "'>" . htmlspecialchars($value['abreviatura']) . "</option>";
                    }
                    ?>
                </select>
            </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        tabela = $('#tabela_dinamica').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: 'controller/pre_distribuicao_ajax.php',
                type: 'GET',
                data: function(d) {
                    ...
                    d.compareceu_designacao_filtro = $('#compareceu_designacao_filtro').val();
                    d.local_compareceu_designacao_filtro = $('#local_compareceu_designacao_filtro').val();
            ...
            columns: [
                ...
                { data: 'nome' },
                { data: 'formacao' },
                { data: 'om_1_fase', orderable: false },
                { data: 'instituicao' },
                { data: 'especialidade' },
                { data: 'ano_residencia', orderable: false },
                { data: 'dias_vida' },
                { data: 'situacao_militar' },
                { data: 'compareceu_desig', orderable: false },
                { data: 'distribuicao', orderable: false },
                { data: 'prioridade_gu', orderable: false },
