<?php
  ...
  $data_apresentacao_atualiza = filtra_campo_post('data_apresentacao_atualiza');
  $om_2_fase_destino = filtra_campo_post('om_2_fase_destino');
  ...
  { 
    ...
    foreach($idsSelecionados as $id_selecionado)
    {
        ...
        $obrigatorio_atualiza->setOm1Fase($om_1_fase);
        $obrigatorio_atualiza->setDistribuicao($distribuicao);
        $obrigatorio_atualiza->setDataProximaApresentacao($data_apresentacao_atualiza);
        $obrigatorio_atualiza->setOm2Fase($om_2_fase_destino);

        $data = $obrigatorioDAO->update($obrigatorio_atualiza);

        if($data)
        {
            $alteracao = "Atualizou o obrigatório na Pré-Distribuição " . $obrigatorio_atualiza;
            ...
            header ("Location: ../pre_distribuicao.php");
        }
        else 
