...
{    
    erro($BASE_URL, 2, 93746514, $pagina_atual, "usuario!admin", "Não foi possível acessar a página!");
    exit();
}
...
$data_selecao_geral  = filtra_campo_post('data_selecao_geral');
...
$data_selecao_geral = trata_data($data_selecao_geral);
...
if($crip != hash('sha256', $_SESSION['chave']. "obrigatorio"))
    erro($BASE_URL, 2, 445235, $pagina_atual, "criptografia_invalida", "Não foi possível editar o usuário!");
...
try
{
    ...
    $obrigatorio_editar->setDataSelecaoGeral($data_selecao_geral);
